...
**UPDATED (2026-08-07, aligned with the CIP):** the express reserve has been
removed. `selectForForge` now FIFO-merges the whole urgent remainder with the
optimistic lane. A certificate-carrying Ranking Block is payload-free: forging
emits an empty transaction sequence and `resolveLeiosBlock` replaces that
sequence with the certified Endorser Block cargo. It never prepends an RB
payload. This removes the parent/child hazard created when a reserved urgent
parent waited for the same certifying block whose EB cargo contained its child,
and makes every transaction in the resolved block share certified delivery.
